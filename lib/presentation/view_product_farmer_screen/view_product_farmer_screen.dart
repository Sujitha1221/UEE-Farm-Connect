    ...
    if (fetchedProdData != null) {
      setState(() {
        ...
        if (fetchedProdData.containsKey("products")) {
          prodList =
              List<Map<String, dynamic>>.from(fetchedProdData["products"]);
        } else {
          prodList =
              []; // Empty list if "products" key is not found in response.
        }
      });
    }
  }
          ...
          child: FloatingActionButton(
            ...
            onPressed: () {
              Navigator.of(context)
                  .pushReplacementNamed('/add_product_page_screen');
            },
            child: Icon(
              Icons.add,
              color: Colors.white,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.pushReplacement(context,
            MaterialPageRoute(builder: (context) => ViewPageFarmerScreen()));
        // You can also update your product list or perform other actions here.
      } else {
        // Handle error responses here
        print(
            'Failed to delete product with ID $id. Status code: ${response.statusCode}');
        print('Response body: ${response.body}');
